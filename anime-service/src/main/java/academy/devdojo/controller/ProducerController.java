...
public class ProducerController {
    ...
    public ResponseEntity<Void> update (@RequestBody ProducerPutRequest request) {
        var produceToRemove = Producer.getProducers()
                .stream()
                .filter(producer -> producer.getId().equals(request.getId()))
                .findFirst()
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Producer not Found"));

        var producerUpdated = MAPPER.toProducer(request, produceToRemove.getCreatedAt());
        Producer.getProducers().remove(produceToRemove);
